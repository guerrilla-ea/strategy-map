...
public class GraphExport implements IModelExporter{

	String MY_EXTENSION = ".cql"; //$NON-NLS-1$
	String MY_EXTENSION_WILDCARD = "*.cql"; //$NON-NLS-1$
	
	
	@Override
	public void export(IArchimateModel model) throws IOException {
		ExportDialog ed = new ExportDialog(Display.getCurrent().getActiveShell(), "Export the selected model to a cypher file.");
		ed.open();
		if(ed.isCancelled()){
			return;
		}
		...
		com.redhat.ea.archimate.basicexport.GraphExporter exportRunner = 
				new com.redhat.ea.archimate.basicexport.GraphExporter(model, file, statuses);
			...
			exportRunner.addJobChangeListener(new JobChangeAdapter(){
				...
				public void done(IJobChangeEvent event) {
					...
					Display.getCurrent().asyncExec(new Runnable(){
						public void run(){
							MessageDialog.openInformation(Display.getCurrent().getActiveShell(), "Cypher Exporter", "Completed Export!");
						}
					});
				}
			});
