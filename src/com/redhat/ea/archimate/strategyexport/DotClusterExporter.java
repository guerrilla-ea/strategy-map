...
public class DotClusterExporter implements IModelExporter {

	String MY_EXTENSION = ".dot"; //$NON-NLS-1$
	String MY_EXTENSION_WILDCARD = "*.dot"; //$NON-NLS-1$


	public DotClusterExporter() {
	}

	@Override
	public void export(IArchimateModel model) throws IOException {
		ExportDialog ed = new ExportDialog(Display.getCurrent().getActiveShell(), "Export the selected Roadmap to a .dot export file\n for visualization with the GraphViz program.");
		ed.open();
		if(ed.isCancelled()){
			return;
		}
		...
		com.redhat.ea.archimate.strategyexport.dependency.DotClusterExporter exportRunner = 
				new com.redhat.ea.archimate.strategyexport.dependency.DotClusterExporter(model, statuses, file);
